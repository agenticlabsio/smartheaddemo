# chatbook
Okay, let's revise the documents focusing strictly on Phase 1 (Integrated Form-Based Booking), integrate it within the Apollo Health application context, use Azure OpenAI as the future AI provider (for Phase 2 description), and significantly expand the user stories and interaction workflows.
...
User logs into Apollo Health App.

User navigates to and clicks "Book Appointment".

HealthConnect module loads. User context (ID, Name, DOB) is passed.

User sees "Who is this appointment for?". User selects "Myself".

Patient Info section appears, pre-filled with User's Name and DOB. User verifies.

User enters "Reason for Appointment".

User selects desired Location and Specialty using dropdowns.

(Optional) User selects a specific Doctor from the filtered list, or leaves as "Any Available".

Loading indicator appears briefly.

Available time slots for the criteria are displayed (e.g., list grouped by date).

User clicks on a desired time slot (e.g., "10:30 AM"). The slot is highlighted.

User clicks "Next" or scrolls to the Summary section.

Summary screen shows all details: Patient=Self (Name, DOB), Reason, Location, Specialty, Doctor, selected Date & Time.

User verifies details and clicks "Confirm Booking".

Brief processing indicator.

Confirmation screen appears: "Appointment Confirmed!" with details and Mock Confirmation ID.

User clicks "Done" or navigates back to the Apollo Health dashboard.
...
HealthConnect module loads. User context (ID) is passed.

User sees "Who is this appointment for?". User selects "Someone Else".

Patient Info section appears with blank, required fields for Name and DOB.

User enters the Patient's Name and Date of Birth.

(Optional) User selects Relationship ("Child").

User enters "Reason for Appointment".

User selects desired Location and Specialty.

(Optional) User selects a specific Doctor.

Available time slots are displayed.

User clicks on a desired time slot.

User proceeds to Summary screen.

Summary screen shows all details: Patient=Someone Else (Entered Name, DOB, Relationship), Reason, Location, Specialty, Doctor, selected Date & Time.

User verifies details and clicks "Confirm Booking".

Confirmation screen appears with details and Mock Confirmation ID.

User clicks "Done" or navigates back.

Workflow 3: Handling "No Availability"

User enters preferences (Location, Specialty, Doctor).

Slot display area shows a message: "No available slots match your criteria. Please try adjusting location, doctor, specialty, or desired date range (if applicable)."

User changes Location preference to a different clinic.

Slot display automatically refreshes (shows loading indicator).

New available slots for the updated criteria are displayed.

User proceeds to select a slot.

Workflow 4: Handling Validation Errors

User is booking for "Someone Else".

User fills in Name but forgets to enter Date of Birth.

User tries to click "Next" or move to the next section.

The Date of Birth field is highlighted (e.g., red border) and an inline message appears: "Date of Birth is required."

User enters the Date of Birth. The error message disappears.

User proceeds.

Workflow 5: Session Restore

User starts booking for self, enters Reason, selects Preferences.

User views available slots but gets interrupted and closes the Apollo Health app/browser tab. (Form state was saved to Redis session in the background).

User reopens the Apollo Health app within 1 hour and navigates back to "Book Appointment".

The system recognizes the active session for the user ID.

The booking form loads, automatically repopulated with the previously selected "Self", pre-filled Name/DOB, entered Reason, and selected Preferences. The available slots section might need reloading or show the previously fetched slots.

User continues the process from selecting a time slot.

Workflow 6: Slot Conflict on Confirmation

User has selected a slot ("11:00 AM") and reviewed the summary.

User clicks "Confirm Booking".

The backend API (POST /api/book-appointment) checks the mock availability and finds the 11:00 AM slot is now marked isAvailable: false (simulated conflict).

The API returns a 409 Conflict error.

The frontend displays a message on the summary screen: "Sorry, the selected time slot (11:00 AM on [Date]) is no longer available. Please select a different time."

The "Confirm Booking" button might be disabled. A "Choose a Different Time" button/link appears.

User clicks the link and is taken back to the slot selection view with updated availability.
